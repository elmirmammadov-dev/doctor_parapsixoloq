            ...
            document.getElementById('campClaimSubmit').addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        overlay.innerHTML = '<div class="camp-modal"><h3 style="text-align:center;color:#27ae60;"><i class="fas fa-check-circle"></i> Təbriklər!</h3><p style="text-align:center;">Sizin ' + data.discount + '% endirim kuponunuz:</p><div class="camp-success-code"><span>' + data.couponCode + '</span></div><button class="camp-copy-btn" onclick="navigator.clipboard.writeText(\\'' + data.couponCode + '\\');this.textContent=\\'Kopyalandı!\\'"><i class="fas fa-copy"></i> Kodu kopyala</button><p style="text-align:center;font-size:0.78rem;color:#999;margin-top:12px;">Endirimi əldə etmək üçün bu kodu WhatsApp-da Şahsəddin İmanlıya göndərin və ya canlı seans zamanı telefonda göstərin.</p><button class="camp-modal-cancel" onclick="this.closest(\\'.camp-modal-overlay\\').remove()" style="margin-top:12px;">Bağla</button></div>';
